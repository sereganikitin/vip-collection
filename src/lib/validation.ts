 ...
 * Возвращает '' если цифр меньше 1.
 */
function normalizeToLocal10(raw: string): string {
  let d = phoneDigits(raw);
  // Снимаем код страны 7 или 8
  if (d.startsWith('7') && d.length === 11) d = d.slice(1);
  else if (d.startsWith('8') && d.length === 11) d = d.slice(1);
  else if (d.startsWith('9') && d.length === 11) d = d.slice(0, 11).slice(1); // на всякий — обрежем «лишнюю» 9 если 11 цифр
  // Иначе оставляем как есть (для маски — частичный ввод)
  return d.slice(0, 10);
}
 ...
 * Правила:
 *   - 10 цифр локальной части после кода страны
...
export function validateRussianPhone(input: string): PhoneValidation {
  const local = normalizeToLocal10(input);
  if (local.length === 0) {
  ...
  if (local === '9876543210' || local === '9123456789') {
    return { ok: false, error: 'Похоже на тестовый номер — введите настоящий' };
  }
  return { ok: true, normalized: '+7' + local };
}
